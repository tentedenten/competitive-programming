struct WeightedEulerTourLCA {
    ...
    WeightedEulerTourLCA(const vector<vector<pair<int,long long>>>& G, int n, int root = 0) {
    ...
    void dfs(const vector<vector<pair<int,long long>>>& G, int u, int p, int d, long long co) {
	...
	long long wdist(int u,int v) const {
